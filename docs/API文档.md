...
- **接口**: `GET /backup/progress`
- **说明**: 备份分三个阶段推进，页面上按秒轮询它画进度条。路径在 `/api/backup/` 下，因此**备份期间同样放行**——否则生成过程中前端拿不到任何响应，进度条只会停在起点。
- **响应**:
```json
{
  "status": 200,
  ...
  "data": {
    ...
    "files_done": 0,
    "files_total": 0,
    "started_at": "2026-09-13T22:30:00",
    "finished_at": null,
    "message": null
  }
}
...
- `phase` 取值 `IDLE` / `COUNTING` / `DUMPING` / `ARCHIVING` / `FINISHED` / `FAILED`。
- 权重：统计范围占前 5%，导出到 80%，剩下 20% 留给附件打包；没有附件时打包阶段直接到位。
- `COUNTING` 阶段按已统计的表数推进；导出阶段按累计行数推进，统计不到行数时退回按表数推进。
- 进度只放在内存里，不落库：它描述的是"此刻"，进程重启后没跑完的备份本来也不会继续。
- **生成期间系统静默**：从开始读取数据到产物写完，除备份接口自身与 `/actuator/**` 外的请求一律返回 `503`（带 `Retry-After: 30`），定时同步任务本次跳过。备份要的是一份停在某一刻的数据，边写边导只会得到半个批次。已经在执行的同步会让备份等它结束再开始，不会被中途打断。
- 产物先落到服务器临时目录再写出，响应结束后立即删除，服务器上不留档。
