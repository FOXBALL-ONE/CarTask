<template>
  <div class="picker">
    ...
    <div v-else aria-label="部门管理范围" class="picker__list" role="group">
      <div v-for="row in flatDepartments" :key="row.department.id" class="picker__row">
        <label :style="{ paddingLeft: `${12 + row.level * 18}px` }" class="picker__item">
          <input
...
.picker__item .material-icons-outlined {
  color: var(--text-mute);
  font-size: 16px;
}
...
.picker__descendants input {
  accent-color: var(--primary);
}

.picker__empty, .picker__hint {
  color: var(--text-mute);
  font-size: 11px;
  line-height: 1.6;
  margin: 0;
