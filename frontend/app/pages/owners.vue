<template>
  <section class="page">
    ...
    <section class="card">
      <div class="toolbar">
        <input v-model="keyword" class="input" placeholder="卡片ID / 户主名 / 手机号" type="search"
               @keyup.enter="search">
        <select v-model="status" aria-label="车主状态" class="select">
      ...
      <div v-else class="table-wrap">
        <table class="table">
          ...
          <tr v-for="owner in owners" :key="owner.id">
            ...
            <td>
              <template v-if="platesByOwner[owner.id]?.length"><span v-for="plate in platesByOwner[owner.id]"
                                                                     :key="plate"
                                                                     class="tag tag--gray plate-tag">{{ plate }}</span>
              </template>
              <span v-else class="muted">{{ owner.plateCount ? `${owner.plateCount} 个` : "-" }}</span></td>
            <td class="muted">-</td>
            <td><span :class="owner.status === 1 ? 'tag--green' : 'tag--red'" class="tag">{{
                owner.status === 1 ? "正常" : "停用"
              }}</span></td>
            <td class="muted">-</td>
            <td class="actions-cell">
              <button v-if="can('owner:manage')" class="row-action" title="充值" type="button"
                      @click="rechargeOwner(owner)"><span class="material-icons-outlined">account_balance_wallet</span>
              </button>
              <button v-if="can('owner:manage')" class="row-action" title="修改信息" type="button"
                      @click="openEdit(owner)"><span class="material-icons-outlined">edit</span></button>
              <button v-if="can('owner:manage')" class="row-action row-action--danger" title="删除" type="button"
                      @click="removeOwner(owner)"><span class="material-icons-outlined">delete</span></button>
            </td>
          </tr>
    ...
    <div v-if="editorVisible" class="modal-mask" @click.self="editorVisible = false">
      <form class="modal" @submit.prevent="saveOwner">
        <header class="modal__head"><h2>{{ editingId ? "编辑车主" : "新增车主" }}</h2>
        ...
        <div class="modal__body">
          <div class="form-grid">
            <label class="field"><span>卡片ID <em>*</em></span><input v-model.trim="form.cardId" class="input" placeholder="如：CAR20240001"
                                                                      required></label>
            <label class="field"><span>户主名 <em>*</em></span><input v-model.trim="form.name" class="input"
                                                                      required></label>
            <label class="field"><span>部门 <em>*</em></span><input v-model.trim="form.dept" class="input"
                                                                    required></label>
            <label class="field"><span>手机号 <em>*</em></span><input v-model.trim="form.phone" class="input" required></label>
            <label class="field"><span>车位数量 <em>*</em></span><input v-model.number="form.spotCount" class="input"
                                                                        min="0" required type="number"></label>
            <label class="field"><span>车牌数量 <em>*</em></span><input v-model.number="form.plateCount" class="input"
                                                                        min="0" required type="number"></label>
            <label class="field"><span>余额</span><input v-model.number="form.balance" class="input" min="0" step="0.01"
                                                         type="number"></label>
...
async function loadOwners() {
  ...
  errorMessage.value = "";
  try {
    const result = await http.get<OwnerList>("/owners", {
...
function openCreate() {
  ...
  Object.assign(form, {cardId: "", name: "", dept: "", phone: "", spotCount: 0, plateCount: 0, balance: 0, status: 1});
  formError.value = "";
...
function openEdit(owner: Owner) {
  ...
  Object.assign(form, {
    ...
    dept: owner.dept,
    phone: owner.phone,
    spotCount: owner.spotCount,
    plateCount: owner.plateCount,
...
async function removeOwner(owner: Owner) {
  ...
  try {
    await http.delete(`/owners/${owner.id}`);
    await loadOwners();
  } catch (error) {
...
async function rechargeOwner(owner: Owner) {
  ...
  if (!Number.isFinite(amount) || amount <= 0) {
    errorMessage.value = "充值金额必须为正数";
    return;
  }
  try {
    await http.post(`/owners/${owner.id}/recharge`, {amount}, {payloadMode: "json"});
    await loadOwners();
  } catch (error) {
    errorMessage.value = (error as { statusMessage?: string }).statusMessage || "充值失败";
  }
}
