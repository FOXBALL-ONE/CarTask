  ...
  <section class="page">
    ...
    <div class="user-layout">
      <aside class="panel department-panel">
        <div class="panel__head department-panel__head">
          <span aria-hidden="true" class="material-icons-outlined">account_tree</span>
          <h2>部门列表</h2>
          <span class="panel__count">共 {{ departments.length }} 个</span>
        </div>
        <div class="department-tree">
          <div :class="{ active: selectedDepartment === 0 }" class="department-row">
            <span aria-hidden="true" class="department-row__spacer"/>
            <button class="department-row__select" type="button" @click="selectDepartment(0)">
              <span aria-hidden="true" class="material-icons-outlined">apartment</span>
              <span class="department-name">全部部门</span>
            </button>
          </div>
          <div aria-hidden="true" class="department-tree__sep"/>
      ...
      <section class="panel user-panel">
        <div class="filter-bar">
          <input v-model="keyword" class="input" placeholder="账号 / 用户名 / 手机号" type="search"
                 @keyup.enter="search">
          <select v-model="status" aria-label="用户状态" class="select">
            <option value="">全部状态</option>
            <option value="1">正常</option>
            <option value="0">停用</option>
          </select>
          <div class="filter-actions">
            <button class="button button--soft" type="button" @click="search"><span class="material-icons-outlined">search</span>搜索
            </button>
        ...
        <div v-else class="table-wrap">
          <table class="table">
            <thead>
            <tr>
              <th class="checkbox-cell"><input v-model="allChecked" aria-label="选择全部用户" class="checkbox"
                                               type="checkbox"></th>
              <th>编号</th>
              <th>账号</th>
              <th>用户名</th>
            ...
            <tr v-for="user in users" :key="user.id">
              <td class="checkbox-cell"><input v-model="selectedIds" :aria-label="`选择${user.name || user.username}`" :value="user.id" class="checkbox"
                                               type="checkbox"></td>
              ...
              <td>
                <button v-if="can('user:disable')" :aria-label="user.status === 1 ? '停用用户' : '启用用户'" :aria-pressed="user.status === 1"
                        :class="{ enabled: user.status === 1 }" class="status-switch"
                        type="button" @click="toggleStatus(user)">
                  <span/>
                </button>
              </td>
              <td class="actions-cell">
                <button v-if="can('user:update')" class="row-action" title="编辑" type="button" @click="openEdit(user)">
                  <span class="material-icons-outlined">edit</span></button>
                <button v-if="can('user:disable')" class="row-action row-action--danger" title="删除" type="button"
                        @click="removeUser(user)"><span class="material-icons-outlined">delete</span></button>
              </td>
            </tr>
        ...
        <footer v-if="!loading && !errorMessage" class="pagination">
          <span>共 {{ total }} 条</span>
          <label class="pagination__size">每页<select v-model.number="pageSize" aria-label="每页展示条数" class="select"
                                                      @change="changePageSize">
            <option v-for="size in pageSizes" :key="size" :value="size">{{ size }} 条</option>
          </select></label>
          <button :disabled="page <= 1" type="button" @click="changePage(page - 1)"><span
    ...
    <div v-if="editorVisible" class="modal-mask" @click.self="editorVisible = false">
      <form class="modal" @submit.prevent="saveUser">
        <header class="modal__head"><h2>{{ editingId ? "编辑用户" : "新增用户" }}</h2>
        ...
        <div class="modal__body">
          <div class="form-grid">
            <label class="field"><span>账号 <em>*</em></span><input v-model.trim="form.username" class="input" placeholder="请输入登录账号"
                                                                    required></label>
            <label class="field"><span>用户名 <em>*</em></span><input v-model.trim="form.name" class="input" placeholder="请输入用户名"
                                                                      required></label>
            <label v-if="!editingId" class="field"><span>初始密码 <em>*</em></span><input v-model="form.password"
                                                                                          class="input" placeholder="请输入初始密码"
                                                                                          required
                                                                                          type="password"></label>
            <label class="field"><span>部门 <em>*</em></span><select v-model="form.deptId" class="select" required>
            ...
            <label class="field"><span>职务</span><input v-model.trim="form.jobTitle" class="input" maxlength="128"
                                                         placeholder="请输入职务"></label>
            <label class="field"><span>角色 <em>*</em></span><select v-model="form.roleId" :disabled="editingId !== null && !canManageScope" :title="editingId !== null && !canManageScope ? '只有超级管理员可以分配角色' : ''"
                                                                     class="select"
                                                                     required>
              <option :value="null">请选择角色</option>
              <option v-for="role in roles" :key="role.id" :value="role.id">{{ role.name }}</option>
            </select></label>
            <label class="field"><span>手机号 <em>*</em></span><input v-model.trim="form.phone" class="input" placeholder="请输入手机号"
                                                                      required></label>
            <label class="field"><span>状态</span><select v-model.number="form.status" :disabled="editingId !== null && !canDisableUser"
                                                          :title="editingId !== null && !canDisableUser ? '当前角色不能启停账号' : ''"
                                                          class="select">
              <option :value="1">正常</option>
              <option :value="0">停用</option>
            </select></label>
...
const allChecked = computed({
  get: () => users.value.length > 0 && users.value.every((user) => selectedIds.value.includes(user.id)),
  set: (checked: boolean) => {
    selectedIds.value = checked ? users.value.map((user) => user.id) : [];
  }
});
...
function roleName(user: User) {
  return roles.value.filter((role) => user.roleIds?.includes(role.id)).map((role) => role.name).join("、") || "未分配";
}

async function loadReferenceData() {
  const [departmentResult, roleResult] = await Promise.all([http.get<Department[]>("/depts"), http.get<RoleList>("/roles", {
    page: 1,
    pageSize: 100
  })]);
...
async function loadUsers() {
  ...
  errorMessage.value = "";
  try {
    const result = await http.get<UserList>("/users", {
...
async function initialize() {
  ...
  } catch { /* The list request reports the visible failure state. */
  }
  await loadUsers();
}
...
function openEdit(user: User) {
  ...
  if (canManageScope.value) {
    void http.get<{ departments: ManagedDepartment[] }>(`/users/${user.id}/managed-departments`)
        .then((result) => {
          managedDepartments.value = result.departments || [];
        })
        .catch(() => {
          managedDepartments.value = [];
        });
  ...
  originalRoleId.value = user.roleIds?.[0] ?? null;
...
async function saveUser() {
  ...
  try {
    if (editingId.value) {
      ...
      if (canDisableUser.value && form.status !== originalStatus.value) await http.put(`/users/${editingId.value}/account-status`, undefined, {
        params: {
          enabled: form.status === 1,
          status: form.status === 1 ? "Activity" : "BANNED"
        }
      });
...
async function toggleStatus(user: User) {
  ...
  try {
    ...
    errorMessage.value = (error as { statusMessage?: string }).statusMessage || "状态更新失败";
  }
}
...
.department-panel__head .material-icons-outlined {
  color: var(--text-mute);
  font-size: 17px;
}
