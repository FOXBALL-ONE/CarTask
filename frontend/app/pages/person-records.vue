<template>
  <section class="page">
    <header class="page__header">
      <div><h1 class="page__title">人员进出记录</h1>
        <p class="page__desc">查看人员通行记录与抓拍信息</p></div>
      <button v-if="can('person-record:export')" class="button button--soft button--sm" type="button"
              @click="exportRecords"><span class="material-icons-outlined">download</span>导出
      </button>
    </header>
    <section class="card">
      <div class="card__body card__body--filters">
        <div class="toolbar"><input v-model.trim="filters.keyword" class="input" placeholder="搜索姓名 / 卡号 / 部门"
        ...
        </select><select v-model="filters.status" aria-label="状态" class="select">
      ...
      <div v-if="loading" class="state">正在加载人员进出记录...</div>
      ...
      <div v-else class="table-wrap">
        <table class="table">
          ...
          <tr v-for="record in pagedRecords" :key="record.id">
            <td><img v-if="record.photo" :alt="record.person" :src="record.photo" class="record-face"
                     loading="lazy"><span v-else class="record-photo record-photo--in"><span
                class="material-icons-outlined">person</span></span></td>
            <td class="person-name">{{ record.person }}</td>
            <td class="text-sub">{{ record.cardId || "-" }}</td>
            <td>{{ record.dept || "-" }}</td>
            <td class="text-sub nowrap">{{ record.time }}</td>
            <td><span :class="record.direction === '进' ? 'tag--blue' : 'tag--gray'" class="tag">{{
                record.direction
              }}</span></td>
            <td>{{ record.gate || "-" }}</td>
            <td>{{ record.method || "-" }}</td>
            <td><span :class="record.status === '正常' ? 'tag--green' : 'tag--red'" class="tag">{{
                record.status
              }}</span></td>
          </tr>
...
const page = ref(1);
const filters = reactive({keyword: "", direction: "", gate: "", status: ""});
const gates = computed(() => [...new Set(records.value.map((record) => record.gate).filter((gate): gate is string => Boolean(gate)))]);
const filteredRecords = computed(() => {
  const keyword = filters.keyword.toLowerCase();
  return records.value.filter((record) => (!keyword || `${record.person}${record.cardId || ""}${record.dept || ""}`.toLowerCase().includes(keyword)) && (!filters.direction || record.direction === filters.direction) && (!filters.gate || record.gate === filters.gate) && (!filters.status || record.status === filters.status));
});
...
function resetFilters() {
  Object.assign(filters, {keyword: "", direction: "", gate: "", status: ""});
  page.value = 1;
  void loadRecords();
}
...
async function loadRecords() {
  ...
  errorMessage.value = "";
  try {
    const result = await http.get<RecordList>("/person-records", {
    ...
    errorMessage.value = (error as { statusMessage?: string }).statusMessage || "人员进出记录加载失败";
  } finally {
    loading.value = false;
  }
}
...
function exportRecords() {
  const rows = [["姓名", "卡片ID", "部门", "进出时间", "方向", "通道", "识别方式", "状态"], ...filteredRecords.value.map((record) => [record.person, record.cardId || "", record.dept || "", record.time, record.direction, record.gate || "", record.method || "", record.status])];
  const csv = rows.map((row) => row.map((cell) => `"${String(cell).replaceAll('"', '""')}"`).join(",")).join("\r\n");
  const link = document.createElement("a");
  link.href = URL.createObjectURL(new Blob(["\ufeff" + csv], {type: "text/csv;charset=utf-8"}));
  link.download = "人员进出记录.csv";
  link.click();
