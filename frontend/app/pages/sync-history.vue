<template>
  <section class="page">
    <header class="page__header">
      <div><h1 class="page__title">同步执行历史</h1>
        <p class="page__desc">数据同步与账号生成任务的近期执行记录，包含手动触发和定时调度。</p></div>
    </header>
    <section class="card">
      <div class="card__body card__body--filters">
        <div class="toolbar"><select v-model="filters.taskKey" aria-label="同步任务" class="select" @change="search">
          <option value="">全部任务</option>
          <option v-for="task in tasks" :key="task.key" :value="task.key">{{ task.label }}</option>
        </select>
          <div class="toolbar__right">
            <button class="button button--soft button--sm" type="button" @click="search"><span
                class="material-icons-outlined">search</span>查询
            </button>
      ...
      <div v-if="loading" class="state">正在加载执行历史...</div>
      ...
      <div v-else class="table-wrap">
        <table class="table">
          ...
          <tr v-for="run in runs" :key="run.id">
            <td>{{ String(run.id).padStart(4, "0") }}</td>
            <td><strong>{{ run.task_name }}</strong></td>
            <td><span :class="run.trigger === 'MANUAL' ? 'tag--blue' : 'tag--gray'" class="tag">{{
                triggerLabel(run.trigger)
              }}</span></td>
            <td><span :class="run.status === 'SUCCESS' ? 'tag--green' : 'tag--red'"
                      class="tag">{{ statusLabel(run.status) }}</span>
            </td>
            <td>{{ run.actor_username }}</td>
            <td class="text-sub nowrap">{{ formatDateTime(run.started_at) }}</td>
            <td><span class="tag tag--gray">{{ formatDuration(run.duration_ms) }}</span></td>
            <td>{{ run.processed_count ?? "—" }}</td>
            <td :title="run.error || run.summary || ''" class="summary-cell"><span
                :class="{ 'summary-cell--error': run.error }">{{ run.error || run.summary || "-" }}</span></td>
          </tr>
