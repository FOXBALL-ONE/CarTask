  ...
  <section class="page">
    ...
    <section class="panel">
      <div class="filter-bar">
        <input v-model.trim="keyword" class="input" placeholder="岗位名称 / 编码" type="search" @keyup.enter="search">
        <select v-model="status" aria-label="岗位状态" class="select">
      ...
      <div v-else class="table-wrap">
        <table class="table">
          ...
          <tr v-for="position in pagedPositions" :key="position.id">
            ...
            <td>{{ position.sort }}</td>
            <td><span :class="position.status === 1 ? 'status-tag--normal' : 'status-tag--disabled'" class="status-tag">{{
                position.status === 1 ? "正常" : "停用"
              }}</span></td>
            <td class="remark">{{ position.remark || "-" }}</td>
            <td class="actions-cell">
              <button v-if="can('position:manage')" class="row-action" title="编辑" type="button"
                      @click="openEdit(position)"><span class="material-icons-outlined">edit</span></button>
              <button v-if="can('position:manage')" class="row-action row-action--danger" title="删除" type="button"
                      @click="removePosition(position)"><span class="material-icons-outlined">delete</span></button>
            </td>
          </tr>
    ...
    <div v-if="editorVisible" class="modal-mask" @click.self="editorVisible = false">
      <form class="modal" @submit.prevent="savePosition">
        ...
        <div class="modal__body">
          <div class="form-grid">
            <label class="field"><span>岗位名称 <em>*</em></span><input v-model.trim="form.name" class="input" placeholder="请输入岗位名称"
                                                                        required></label>
            <label class="field"><span>岗位编码 <em>*</em></span><input v-model.trim="form.code" class="input" placeholder="请输入岗位编码"
                                                                        required></label>
            <label class="field"><span>显示排序</span><input v-model.number="form.sort" class="input" min="1"
                                                             type="number"></label>
            <label class="field"><span>状态</span><select v-model.number="form.status" class="select">
              <option :value="1">正常</option>
              <option :value="0">停用</option>
            </select></label>
            <label class="field field--full"><span>备注</span><textarea v-model.trim="form.remark" class="textarea"
...
.code-tag::before, .status-tag::before {
  border-radius: 50%;
  content: "";
  height: 5px;
  width: 5px;
