...
.login__input-wrap .material-icons-outlined {
  color: var(--g-mute);
  font-size: 19px;
  left: 12px;
  position: absolute;
...
.login__captcha-row .login__input-wrap {
  flex: 1;
}
...
.login__sms-btn:hover:not(:disabled) {
  background: rgb(37 99 235 / 14%);
  border-color: rgb(37 99 235 / 45%);
}

.login__sms-btn:disabled {
  background: var(--g-surface);
...
.login__btn:hover:not(:disabled) {
  box-shadow: 0 10px 26px rgb(37 99 235 / 34%);
...
@media (max-width: 768px) {
  ...
  .login__input, .login__btn, .login__captcha, .login__sms-btn {
    font-size: 16px;
    height: 48px;
  }
