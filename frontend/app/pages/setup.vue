  ...
  <div class="setup-page">
    ...
    <div v-if="phase === 'locked'" class="gate">
      <div class="gate__card">
        ...
        <button class="btn btn--primary" type="button" :disabled="busy || !tokenInput.trim()" @click="unlock">
          {{ busy ? "校验中..." : "进入配置引导" }}
        </button>
    ...
    <div v-else-if="phase === 'already'" class="gate">
      <div class="gate__card">
        <div class="gate__head">
          <div class="mark mark--ok" aria-hidden="true"><span class="material-icons-outlined">verified</span></div>
          <div>
    ...
    <div v-else-if="phase === 'restarting'" class="gate">
      <div class="gate__card">
        <div class="gate__head">
          <div class="mark" :class="restartTimedOut ? 'mark--warn' : 'mark--spin'" aria-hidden="true">
        ...
        <p class="gate__hint">
          配置已写入 <code>{{ envFile }}</code><span v-if="backupFile">，原文件备份为 <code>{{ backupFile }}</code></span>。
        </p>
      ...
      <section class="panel">
        ...
        <div v-if="currentStep.id === 'database'" class="form">
          <label class="field">
            ...
            <div class="field__wrap">
              ...
              <input v-model="database.url" class="field__input" spellcheck="false" placeholder="jdbc:postgresql://主机:5432/cartask">
            </div>
          ...
          <div class="form__row">
            <label class="field">
              ...
              <div class="field__wrap">
                ...
                <input v-model="database.username" class="field__input" autocomplete="off" placeholder="postgres">
              </div>
            </label>
            <label class="field">
              ...
              <div class="field__wrap">
                ...
                <input v-model="database.password" class="field__input" type="password" autocomplete="new-password" placeholder="没有密码可留空">
              </div>
            </label>
          </div>
        </div>
        ...
        <div v-else-if="currentStep.id === 'redis'" class="form">
          <div class="form__row form__row--host">
            <label class="field">
              ...
              <div class="field__wrap">
                ...
                <input v-model="redis.host" class="field__input" spellcheck="false" placeholder="127.0.0.1">
              </div>
            </label>
            <label class="field">
          ...
          <label class="field">
            ...
            <div class="field__wrap">
              <span class="material-icons-outlined">lock</span>
              <input v-model="redis.password" class="field__input" type="password" autocomplete="new-password" placeholder="未启用鉴权可留空">
            </div>
        ...
        <div v-else-if="currentStep.id === 'keytop'" class="form">
          <label class="field">
            ...
            <div class="field__wrap">
              ...
              <input v-model="keytop.base_url" class="field__input" spellcheck="false" placeholder="https://kp-open.keytop.cn/unite-api">
            </div>
          </label>
          <div class="form__row">
            <label class="field">
              ...
              <div class="field__wrap">
                ...
                <input v-model="keytop.app_id" class="field__input" inputmode="numeric" placeholder="科拓分配的应用编号">
              </div>
            </label>
            <label class="field">
              ...
              <div class="field__wrap">
                ...
                <input v-model="keytop.park_id" class="field__input" spellcheck="false" placeholder="科拓分配的车场编号">
              </div>
            </label>
          </div>
          <label class="field">
            ...
            <div class="field__wrap">
              ...
              <input v-model="keytop.app_secret" class="field__input" type="password" autocomplete="new-password" placeholder="平台分配的密钥">
            </div>
        ...
        <div v-else-if="currentStep.id === 'storage'" class="form">
          <label class="field">
            ...
            <div class="field__wrap">
              ...
              <input v-model="storage.storage_root" class="field__input" spellcheck="false" placeholder="./st">
            </div>
          ...
          <label class="field">
            ...
            <div class="field__wrap">
              ...
              <input v-model="storage.base_url" class="field__input" spellcheck="false" placeholder="http://192.168.1.95:8080">
            </div>
        ...
        <div v-else-if="currentStep.id === 'sms'" class="form">
          ...
          <div class="form__row">
            <label class="field">
              ...
              <div class="field__wrap">
                <span class="material-icons-outlined">badge</span>
                <input v-model="sms.access_key_id" class="field__input" autocomplete="off" spellcheck="false" placeholder="阿里云 AccessKey ID">
              </div>
            </label>
            <label class="field">
              ...
              <div class="field__wrap">
                <span class="material-icons-outlined">vpn_key</span>
                <input v-model="sms.access_key_secret" class="field__input" type="password" autocomplete="new-password" placeholder="阿里云 AccessKey Secret">
              </div>
            </label>
          </div>
          <div class="form__row">
            ...
            <label class="field">
              ...
              <div class="field__wrap">
                ...
                <input v-model="sms.template_code" class="field__input" spellcheck="false" placeholder="SMS_xxxxxxxxx">
              </div>
            </label>
          </div>
          <label class="field">
            ...
            <div class="field__wrap">
              ...
              <input v-model="sms.phone" class="field__input" type="tel" inputmode="tel" placeholder="例如 13800000000">
            </div>
        ...
        <div v-else-if="currentStep.id === 'administrator'" class="form">
          ...
          <label class="field">
            ...
            <div class="field__wrap">
              ...
              <input v-model="administrator.username" class="field__input" autocomplete="off" spellcheck="false" placeholder="admin">
            </div>
          ...
          <div class="form__row">
            <label class="field">
              ...
              <div class="field__wrap">
                <span class="material-icons-outlined">lock</span>
                <input v-model="administrator.password" class="field__input" type="password" autocomplete="new-password" placeholder="至少 8 位">
              </div>
            </label>
            <label class="field">
              ...
              <div class="field__wrap">
                ...
                <input v-model="administrator.confirm_password" class="field__input" type="password" autocomplete="new-password" placeholder="再输入一次">
              </div>
            </label>
          </div>
          <p class="field__hint">
        ...
        <footer class="panel__foot">
          ...
          <button v-if="currentStep.id === 'review'" class="btn btn--primary" type="button" :disabled="busy" @click="finish">
            {{ busy ? "正在写入..." : "写入配置并启动" }}
          </button>
          <button v-else class="btn btn--primary" type="button" :disabled="busy" @click="next">
            {{ nextLabel }}
          </button>
        </footer>
      </section>
...
import {useSetup, type SetupStepId} from "~/composables/useSetup";
...
.mark { flex: 0 0 auto; height: 46px; position: relative; width: 54px; }
.mark__lane { background: var(--s-line-strong); bottom: 4px; height: 1px; left: 0; position: absolute; right: 0; }
.mark__post { background: var(--s-post); border-radius: 2px; bottom: 4px; height: 16px; position: absolute; right: 0; width: 6px; }
.mark__arm {
...
.mark--spin { animation: setupSpin 1.6s linear infinite; color: var(--s-accent-text); background: var(--s-surface); border-color: var(--s-line-strong); }
.mark--warn {
  align-items: center;
...
/* ====== 口令闸门 / 完成提示：单卡片居中 ====== */
.gate { position: relative; z-index: 1; width: 440px; max-width: 100%; }
.gate__card {
...
@keyframes setupIn { from { opacity: 0; transform: translateY(14px); } to { opacity: 1; transform: translateY(0); } }
.gate__head { align-items: center; display: flex; gap: 14px; margin-bottom: 22px; }
.gate__title { color: var(--s-text); font-size: 19px; font-weight: 700; margin: 0; }
.gate__desc { color: var(--s-mute); font-size: 12px; margin: 5px 0 0; }
.gate__hint { color: var(--s-mute); font-size: 12px; line-height: 1.7; margin: 16px 0 0; }
...
.brand { padding: 0 6px 20px; }
.brand__text { min-width: 0; }
.brand__title { color: var(--s-text); font-size: 15px; font-weight: 700; margin: 0; }
.brand__desc { color: var(--s-mute); font-size: 11px; letter-spacing: .06em; margin: 3px 0 0; }

.steps { flex: 1; list-style: none; margin: 0; padding: 0; }
...
.step--done:not(:last-child)::after { background: rgb(5 150 105 / 35%); }
...
.step__button:hover { background: var(--s-surface); }
.step__marker {
  align-items: center;
  background: var(--s-panel);
  border: 1px solid var(--s-line-strong);
...
.step__marker .material-icons-outlined { font-size: 14px; }
.step__text { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
.step__title { color: var(--s-sub); font-size: 13px; font-weight: 500; }
.step__hint { color: var(--s-mute); font-size: 11px; }

.step--done .step__marker { background: var(--s-ok); border-color: var(--s-ok); color: #fff; }
.step--done .step__title { color: var(--s-text); }
.step--active .step__button { background: var(--s-panel); box-shadow: 0 1px 3px rgb(15 23 42 / 10%); }
.step--active .step__marker { border-color: var(--s-accent); box-shadow: 0 0 0 3px rgb(59 130 246 / 15%); color: var(--s-accent-text); }
.step--active.step--done .step__marker { background: var(--s-ok); border-color: var(--s-ok); box-shadow: 0 0 0 3px rgb(5 150 105 / 15%); color: #fff; }
.step--active .step__title { color: var(--s-accent-text); font-weight: 600; }

.rail__note { border-top: 1px solid var(--s-line); color: var(--s-mute); font-size: 11px; line-height: 1.8; margin: 16px 0 0; padding: 14px 6px 0; }

/* ====== 右侧面板 ====== */
.panel { display: flex; flex-direction: column; min-width: 0; padding: 26px 30px 22px; }
.panel__head { margin-bottom: 20px; }
.panel__title { color: var(--s-text); font-size: 18px; font-weight: 700; margin: 0; }
.panel__desc { color: var(--s-sub); font-size: 12.5px; line-height: 1.7; margin: 6px 0 0; }
.panel__foot { align-items: center; display: flex; gap: 10px; margin-top: auto; padding-top: 22px; }
...
.form__row--host { grid-template-columns: minmax(0, 1fr) 118px; }
.form__note { background: var(--s-surface); border: 1px solid var(--s-line); border-radius: 9px; color: var(--s-sub); font-size: 12.5px; line-height: 1.75; margin: 0; padding: 12px 14px; }

.field { display: block; min-width: 0; }
.field__label { color: var(--s-sub); display: block; font-size: 12px; font-weight: 500; margin-bottom: 6px; }
.field__wrap { position: relative; }
.field__wrap .material-icons-outlined { color: var(--s-mute); font-size: 18px; left: 12px; position: absolute; top: 50%; transform: translateY(-50%); }
.field__input {
  background: var(--s-panel);
  border: 1px solid var(--s-line-strong);
  border-radius: 9px;
...
.field__input--token { font-family: ui-monospace, Consolas, 'SFMono-Regular', monospace; font-size: 16px; letter-spacing: .22em; text-transform: uppercase; }
.field__input::placeholder { color: var(--s-mute); opacity: 1; }
/* 字距是为了让抄进来的口令逐位可读；占位符不是口令，被拉开只会显得排版出错。 */
.field__input--token::placeholder { font-family: inherit; font-size: 13px; letter-spacing: normal; text-transform: none; }
.field__input:focus-visible, .field__input:focus { border-color: var(--s-accent); box-shadow: 0 0 0 3px rgb(59 130 246 / 15%); }
.field__hint { color: var(--s-mute); display: block; font-size: 11.5px; line-height: 1.7; margin-top: 6px; }
...
.btn--primary { background: linear-gradient(180deg, var(--s-accent-deep), var(--s-accent-text)); box-shadow: 0 8px 20px rgb(37 99 235 / 24%); color: #fff; }
.btn--primary:hover:not(:disabled) { box-shadow: 0 10px 26px rgb(37 99 235 / 32%); filter: brightness(1.05); }
.btn--ghost { background: var(--s-panel); border-color: var(--s-line-strong); color: var(--s-sub); }
.btn--ghost:hover:not(:disabled) { background: var(--s-surface); color: var(--s-text); }
.btn:disabled { box-shadow: none; cursor: not-allowed; opacity: .5; }
.gate__card .btn { width: 100%; }

/* ====== 提示条 ====== */
.alert { border-radius: 9px; font-size: 12.5px; line-height: 1.7; margin: 16px 0 0; padding: 10px 13px; }
.alert--error { background: var(--s-danger-bg); border: 1px solid var(--s-danger-border); color: var(--s-danger); }
.alert--ok { background: var(--s-ok-bg); border: 1px solid rgb(5 150 105 / 22%); color: #047857; }
.gate__card .alert { margin-bottom: 0; }

/* ====== 确认页摘要 ====== */
.review { display: grid; gap: 1px; margin: 0; background: var(--s-line); border: 1px solid var(--s-line); border-radius: 9px; overflow: hidden; }
.review dt, .review dd { background: var(--s-panel); font-size: 12.5px; margin: 0; padding: 9px 13px; }
.review dt { color: var(--s-mute); }
.review dd { color: var(--s-text); overflow-wrap: anywhere; }
.review dd code { background: none; border: 0; padding: 0; font-size: 12.5px; }
.review { grid-template-columns: 132px minmax(0, 1fr); }
.chip { background: var(--s-ok-bg); border: 1px solid rgb(5 150 105 / 22%); border-radius: 4px; color: #047857; font-size: 11px; padding: 2px 8px; }

/* 键盘可达性 */
.step__button:focus-visible, .btn:focus-visible { outline: 2px solid var(--s-accent); outline-offset: 2px; }

/* ====== 窄屏：轨道改成横向条，面板占满 ====== */
@media (max-width: 860px) {
  .setup-page { align-items: flex-start; padding: 16px; overflow: auto; }
  .setup-page::before { display: none; }
  .shell { grid-template-columns: minmax(0, 1fr); min-height: 0; }
  .rail { border-right: 0; border-bottom: 1px solid var(--s-line); padding: 18px 16px 12px; }
  .brand { padding-bottom: 14px; }
  .steps { display: flex; gap: 4px; overflow-x: auto; padding-bottom: 4px; }
  .step:not(:last-child)::after { display: none; }
  .step__button { flex-direction: column; gap: 6px; min-width: 76px; padding: 6px 4px; text-align: center; }
  .step__text { align-items: center; }
...
@media (max-width: 560px) {
  ...
  .review dt { padding-bottom: 2px; }
  .review dd { padding-top: 2px; }
  .panel__foot { flex-wrap: wrap; }
  .panel__foot .btn { flex: 1; padding: 0 14px; }
...
@media (prefers-reduced-motion: reduce) {
  .gate__card, .shell, .mark--spin { animation: none; }
}
</style>
