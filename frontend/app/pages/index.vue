<template>
  <section class="page">
    <header class="page__header">
      ...
      <button :disabled="loading" class="refresh-button" type="button" @click="loadDashboard">
    ...
    <template v-else>
      <div class="stat-grid">
        <article v-for="stat in dashboard?.stats" :key="stat.label" :class="`stat-card--${stat.color}`"
                 class="stat-card">
          <strong class="stat-card__value">{{ stat.value }}</strong>
          <span class="stat-card__label">{{ stat.label }}</span>
          <span :class="stat.trend" class="stat-card__delta">{{ stat.delta }}</span>
        </article>
      </div>
        ...
        <article class="card chart-card">
          ...
          <div v-if="parkingAreas.length" class="bars">
            <div v-for="area in parkingAreas" :key="area.area" class="bar-col">
              <div class="bar-stack">
                <div :style="{ height: `${usedHeight(area)}%` }" class="bar-seg bar-used"/>
                <div :style="{ height: `${freeHeight(area)}%` }" class="bar-seg bar-free"/>
                <div class="bar-hover" @mouseenter="showBarTip(area, $event)" @mouseleave="hideTip"
                     @mousemove="moveTip"/>
              </div>
          ...
          <div v-if="donutSlices.length" class="donut-layout">
            <div class="donut-legend">
              <div v-for="(slice, index) in donutSlices" :key="slice.name"
                   :class="{ 'legend-row--bordered': index > 0 }"
                   class="legend-row">
                <span :style="{ background: slice.color }" class="legend__dot legend__dot--round"/>
            ...
            <div class="donut-svg">
              <svg aria-label="违规类型占比" role="img" style="width: 180px; height: 180px; display: block;"
                   viewBox="0 0 180 180">
                <path v-for="slice in donutSlices" :key="`arc-${slice.name}`" :d="slice.path" :fill="slice.color"
                      stroke="var(--card)" stroke-width="2"/>
                <text fill="var(--text)" font-size="24" font-weight="700" text-anchor="middle" x="90" y="85">
                  {{ violationTotal }}
                </text>
                <text fill="var(--text-sub)" font-size="11" text-anchor="middle" x="90" y="105">违规总数</text>
              </svg>
            </div>
          </div>
      ...
      <div class="dash-grid dash-grid--trends">
          ...
          <div class="chart-card__head">
            ...
            <div v-if="violationTrend" class="chart-legend">
              <span v-for="series in violationTrend.series" :key="series.name" class="legend__item">
                <span :style="{ background: series.color }" class="legend__dot legend__dot--round"/>{{ series.name }}
              </span>
            </div>
          </div>
          <div class="line-chart">
            <canvas ref="violationCanvas" class="line-canvas" @mouseleave="hideTip" @mousemove="handleLineHover"/>
          </div>
        </article>
        ...
        <article class="card chart-card">
          ...
          <div class="line-chart">
            <canvas ref="inoutCanvas" class="line-canvas" @mouseleave="hideTip" @mousemove="handleLineHover"/>
          </div>
        </article>
      </div>

      <div v-show="tip.visible" :style="{ left: `${tip.x}px`, top: `${tip.y}px` }" aria-hidden="true" class="chart-tip">
...
function drawLineChart(canvas: HTMLCanvasElement, series: TrendSeries[], labels: string[]) {
  ...
  const coords = series.map((item) => item.data.map((value, index) => ({
    x: toX(index),
    y: toY(value),
    val: value,
    label: labels[index] ?? ""
