<template>
  <section class="sync-page">
    ...
    <article v-if="can('dictionary:sync')" :class="`sync-card--${parkingAreaCardState}`" class="sync-card">
      <div class="sync-card__main">
        <header class="sync-card__header">
          ...
          <div>
            <div class="sync-card__title-row">
              <h2>停车区域</h2>
              <span aria-live="polite" class="state-badge">{{ parkingAreaStateLabel }}</span>
            </div>
            <p>按科拓区域编码匹配本地数据；新增缺失区域，并更新已有区域的名称和顺序。</p>
            <div v-if="progressFor('parking_area.sync')?.running" class="progress">
              <div :style="{ width: progressPercent('parking_area.sync') + '%' }" class="progress__bar"/>
              <span>{{ progressText('parking_area.sync') }}</span></div>
          </div>
        </header>

        <div :class="{ 'sync-route--running': parkingAreaRunning }" class="sync-route">
          <div class="endpoint">
            <span class="endpoint__mark endpoint__mark--remote"><span
                class="material-icons-outlined">cloud</span></span>
            <span><small>数据来源</small><strong>科拓开放平台</strong></span>
          </div>
          <div aria-hidden="true" class="conduit">
        ...
        <div v-if="syncState === 'error' && !parkingAreaRunning" class="feedback feedback--error" role="alert">
          ...
          <div><strong>同步未完成</strong>
            <p>{{ errorMessage }}</p></div>
        </div>
        <div v-else-if="syncState === 'success' && !parkingAreaRunning" class="feedback feedback--success"
             role="status">
          <span class="material-icons-outlined">check_circle_outline</span>
          <div><strong>同步完成</strong>
            <p>{{ resultSummary }}</p></div>
        </div>
      </div>
      ...
      <aside class="sync-card__action">
        <template v-if="!confirming">
          ...
          <button :disabled="parkingAreaRunning" class="sync-button" type="button" @click="confirming = true">
            <span :class="{ spinning: parkingAreaRunning }"
                  class="material-icons-outlined">{{ parkingAreaRunning ? 'sync' : 'sync_alt' }}</span>
        ...
        <template v-else>
          ...
          <div class="confirm-actions">
            ...
            <button :disabled="parkingAreaRunning" class="sync-button" type="button" @click="runSynchronization">
    ...
    <article v-if="can('vehicle-record:sync')" :class="`sync-card--${accessRecordCardState}`"
      ...
      <div class="sync-card__main">
        <header class="sync-card__header">
          ...
          <div>
            <div class="sync-card__title-row">
              <h2>车辆进出记录</h2>
              <span aria-live="polite" class="state-badge">{{ accessRecordStateLabel }}</span>
            </div>
            <p>以最近一次成功快照为起点增量拉取车辆进出记录，并将抓拍图片保存到本地文件库。</p>
            <div v-if="progressFor('car_cap_info.sync', 'car_cap_info.reconciliation')?.running" class="progress">
              <div :style="{ width: progressPercent('car_cap_info.sync', 'car_cap_info.reconciliation') + '%' }"
                   class="progress__bar"/>
              <span>{{ progressText('car_cap_info.sync', 'car_cap_info.reconciliation') }}</span></div>
          </div>
        </header>

        <div :class="{ 'sync-route--running': accessRecordRunning || accessRecordPreviewing }" class="sync-route">
          <div class="endpoint">
            <span class="endpoint__mark endpoint__mark--remote"><span
                class="material-icons-outlined">photo_camera</span></span>
        ...
        <div v-if="accessRecordSyncState === 'error' && !accessRecordRunning" class="feedback feedback--error"
             role="alert">
          <span class="material-icons-outlined">error_outline</span>
          <div><strong>同步未完成</strong>
            <p>{{ accessRecordErrorMessage }}</p></div>
        </div>
        <div v-else-if="accessRecordSyncState === 'success' && !accessRecordRunning" class="feedback feedback--success"
             role="status">
          <span class="material-icons-outlined">check_circle_outline</span>
          <div><strong>同步完成</strong>
            <p>{{ accessRecordResultSummary }}</p></div>
        </div>
        <div v-else-if="accessRecordPreview && accessRecordSyncState === 'idle'" class="feedback feedback--preview"
             role="status">
          <span class="material-icons-outlined">fact_check</span>
          <div><strong>同步预检完成</strong>
            <p>{{ accessRecordPreviewSummary }}</p></div>
        </div>
      </div>
      ...
      <aside class="sync-card__action">
        <template v-if="accessRecordPreviewing || accessRecordRunning">
          ...
          <p class="action-copy">{{
              accessRecordPreviewing ? "正在读取科拓待同步记录数，尚未写入本地数据。" : "正在保存流水并下载可用的抓拍图片。"
            }}</p>
          <button class="sync-button" disabled type="button">
            <span class="material-icons-outlined spinning">sync</span>{{
              accessRecordPreviewing ? "正在获取..." : "正在同步..."
            }}
          </button>
        </template>
        ...
        <template v-else>
          ...
          <p class="action-copy">{{
              accessRecordPreview.pending_count === null ? "科拓未返回总数；确认后将按预检时间范围同步。" : `预检发现 ${accessRecordPreview.pending_count} 条记录，确认后开始同步。`
            }}</p>
          <div class="confirm-actions">
    ...
    <article v-if="can('owner:sync')" :class="`sync-card--${ownerArchiveCardState}`"
             class="sync-card sync-card--owners">
      <div class="sync-card__main">
        <header class="sync-card__header">
          ...
          <div>
            <div class="sync-card__title-row">
              <h2>车主信息补建</h2>
              <span aria-live="polite" class="state-badge">{{ ownerArchiveStateLabel }}</span>
            </div>
            <p>以近 30
              天有进出记录的车牌为依据（自动去重），对系统没有车主信息的车牌回查科拓卡片信息，新建车主信息并登记车牌关联。</p>
            <div v-if="progressFor('owner.archive.generate')?.running" class="progress">
              <div :style="{ width: progressPercent('owner.archive.generate') + '%' }" class="progress__bar"/>
              <span>{{ progressText('owner.archive.generate') }}</span></div>
          </div>
        </header>

        <div :class="{ 'sync-route--running': ownerArchiveRunning }" class="sync-route">
          <div class="endpoint">
        ...
        <div v-if="ownerArchiveSyncState === 'error' && !ownerArchiveRunning" class="feedback feedback--error"
             role="alert">
          <span class="material-icons-outlined">error_outline</span>
          <div><strong>补建未完成</strong>
            <p>{{ ownerArchiveErrorMessage }}</p></div>
        </div>
        <div v-else-if="ownerArchiveSyncState === 'success' && !ownerArchiveRunning" class="feedback feedback--success"
             role="status">
          <span class="material-icons-outlined">check_circle_outline</span>
          <div><strong>补建完成</strong>
            <p>{{ ownerArchiveResultSummary }}</p></div>
        </div>
      </div>
      ...
      <aside class="sync-card__action">
        <template v-if="!ownerArchiveConfirming">
          ...
          <button :disabled="ownerArchiveRunning" class="sync-button" type="button"
                  @click="ownerArchiveConfirming = true">
            <span :class="{ spinning: ownerArchiveRunning }"
                  class="material-icons-outlined">{{ ownerArchiveRunning ? 'sync' : 'person_search' }}</span>
        ...
        <template v-else>
          <p class="action-label">确认执行</p>
          <p class="action-copy">
            新建的车主信息以科拓卡片的卡号、姓名和手机号为准，同一手机号的多个车牌只建一条车主信息。已停用的档案和车主不会被改动。</p>
          <div class="confirm-actions">
            <button class="cancel-button" type="button" @click="ownerArchiveConfirming = false">取消</button>
            <button :disabled="ownerArchiveRunning" class="sync-button" type="button"
                    @click="runOwnerArchiveGeneration">
    ...
    <article v-if="can('account:sync')" :class="`sync-card--${accountCardState}`" class="sync-card sync-card--accounts">
      <div class="sync-card__main">
        <header class="sync-card__header">
          ...
          <div>
            <div class="sync-card__title-row">
              <h2>车辆业主账号</h2>
              <span aria-live="polite" class="state-badge">{{ accountStateLabel }}</span>
            </div>
            <p>以近 30
              天有进出记录的车牌为依据（自动去重），经车牌档案关联在营车主，为尚未注册的业主自动创建平台登录账号。车主信息缺失的车牌由「车主信息补建」先行处理。</p>
            <div v-if="progressFor('account.generate')?.running" class="progress">
              <div :style="{ width: progressPercent('account.generate') + '%' }" class="progress__bar"/>
              <span>{{ progressText('account.generate') }}</span></div>
          </div>
        </header>

        <div :class="{ 'sync-route--running': accountRunning }" class="sync-route">
          <div class="endpoint">
            <span class="endpoint__mark endpoint__mark--remote"><span
                class="material-icons-outlined">directions_car</span></span>
        ...
        <div v-if="accountSyncState === 'error' && !accountRunning" class="feedback feedback--error" role="alert">
          <span class="material-icons-outlined">error_outline</span>
          <div><strong>生成未完成</strong>
            <p>{{ accountErrorMessage }}</p></div>
        </div>
        <div v-else-if="accountSyncState === 'success' && !accountRunning" class="feedback feedback--success"
             role="status">
          <span class="material-icons-outlined">check_circle_outline</span>
          <div><strong>生成完成</strong>
            <p>{{ accountResultSummary }}</p></div>
        </div>
      </div>
      ...
      <aside class="sync-card__action">
        <template v-if="!accountConfirming">
          ...
          <button :disabled="accountRunning" class="sync-button" type="button" @click="accountConfirming = true">
            <span :class="{ spinning: accountRunning }"
                  class="material-icons-outlined">{{ accountRunning ? 'sync' : 'person_add_alt' }}</span>
        ...
        <template v-else>
          <p class="action-label">确认执行</p>
          <p class="action-copy">
            将以业主手机号作为登录名创建账号，并按车主信息的部门字段挂靠部门（缺同名部门时自动新建）。已存在的账号自动跳过，不会重复创建。同一业主的多个车牌只处理一次。</p>
          <div class="confirm-actions">
            <button class="cancel-button" type="button" @click="accountConfirming = false">取消</button>
            <button :disabled="accountRunning" class="sync-button" type="button" @click="runAccountGeneration">
    ...
    <section v-if="can('vehicle-record:sync')" aria-live="polite" class="result-panel result-panel--records">
      <header class="result-panel__header">
        <div>
          <p class="result-panel__label">最近一次进出记录同步</p>
          <p class="result-panel__time">
            {{ accessRecordResult ? formatDateTime(accessRecordResult.executed_at) : "执行后将在这里显示本次统计" }}</p>
        </div>
        <span v-if="accessRecordResult"
              class="result-panel__total">新游标 {{ formatDateTime(accessRecordResult.cursor_time) }}</span>
      </header>
      <div class="metric-grid">
        <div class="metric"><span>处理流水</span><strong>{{ accessRecordResult?.processed_count ?? "—" }}</strong></div>
        <div class="metric metric--photo"><span>图片落地</span><strong>{{
            accessRecordResult?.local_photo_count ?? "—"
          }}</strong></div>
        <div class="metric metric--retry">
          <span>待重试图片</span><strong>{{ accessRecordResult?.failed_photo_count ?? "—" }}</strong></div>
      </div>
    </section>

    <section v-if="can('owner:sync')" aria-live="polite" class="result-panel result-panel--owners">
      <header class="result-panel__header">
        <div>
          <p class="result-panel__label">最近一次车主信息补建</p>
          <p class="result-panel__time">
            {{ ownerArchiveResult ? formatDateTime(ownerArchiveResult.executed_at) : "执行后将在这里显示本次统计" }}</p>
        </div>
        <div class="result-links">
          <NuxtLink class="result-link" to="/owners"><span class="material-icons-outlined">badge</span>车主信息
          </NuxtLink>
          <NuxtLink class="result-link" to="/plates"><span class="material-icons-outlined">directions_car</span>车牌信息
          </NuxtLink>
        </div>
      </header>
      <div class="metric-grid">
        <div class="metric metric--owner"><span>新建车主</span><strong>{{
            ownerArchiveResult?.created_owner_count ?? "—"
          }}</strong></div>
        <div class="metric metric--linked"><span>关联车牌</span><strong>{{
            ownerArchiveResult?.linked_plate_count ?? "—"
          }}</strong></div>
        <div class="metric metric--quiet"><span>跳过</span><strong>{{
            ownerArchiveResult?.skipped_count ?? "—"
          }}</strong></div>
      </div>
    </section>

    <section v-if="can('owner:sync')" aria-live="polite" class="history-panel">
      <header class="history-panel__header">
        <p class="history-panel__label">车主信息补建 · 近期执行</p>
        <NuxtLink class="history-panel__link" to="/sync-history">全部记录<span class="material-icons-outlined">chevron_right</span>
        </NuxtLink>
      </header>
      <p v-if="historyUnavailable" class="history-panel__empty">执行历史读取失败，需要「查看同步执行历史」权限。</p>
      <p v-else-if="ownerArchiveHistory.length === 0" class="history-panel__empty">暂无执行记录，定时任务每天 02:45
        执行。</p>
      <ul v-else class="history-list">
        <li v-for="run in ownerArchiveHistory" :key="run.id" class="history-list__row">
          ...
          <span :class="run.trigger === 'MANUAL' ? 'history-chip--manual' : 'history-chip--schedule'"
                class="history-chip">{{
              run.trigger === "MANUAL" ? "手动" : "定时"
            }}</span>
          <span :class="run.status === 'SUCCESS' ? 'history-chip--ok' : 'history-chip--fail'" class="history-chip">{{
              run.status === "SUCCESS" ? "成功" : "失败"
            }}</span>
          <span :title="run.error || run.summary || ''" class="history-list__summary">{{
    ...
    <section v-if="can('account:sync')" aria-live="polite" class="result-panel result-panel--accounts">
      <header class="result-panel__header">
        <div>
          ...
          <p class="result-panel__time">{{
              accountResult ? formatDateTime(accountResult.executed_at) : "执行后将在这里显示本次统计"
            }}</p>
        </div>
      ...
      <div class="metric-grid">
        ...
        <div class="metric metric--quiet"><span>跳过</span><strong>{{ accountResult?.skipped_count ?? "—" }}</strong>
        </div>
        <div class="metric metric--retry"><span>失败</span><strong>{{ accountResult?.failed_count ?? "—" }}</strong>
        </div>
      </div>
    </section>

    <section v-if="can('account:sync')" aria-live="polite" class="history-panel">
      <header class="history-panel__header">
    ...
    <section v-if="can('dictionary:sync')" aria-live="polite" class="result-panel">
      <header class="result-panel__header">
        <div>
          <p class="result-panel__label">最近一次手动结果</p>
          <p class="result-panel__time">{{
              result ? formatDateTime(result.executed_at) : "执行后将在这里显示本次统计"
            }}</p>
        </div>
    ...
    <section v-if="can('sync-schedule:manage')" class="schedule-panel">
      ...
      <div v-if="scheduleError" class="feedback feedback--error" role="alert">
        <span class="material-icons-outlined">error_outline</span>
        <div><strong>周期未保存</strong>
          <p>{{ scheduleError }}</p></div>
      </div>
      <div v-else-if="scheduleMessage" class="feedback feedback--success" role="status">
        <span class="material-icons-outlined">check_circle_outline</span>
        <div><strong>周期已生效</strong>
          <p>{{ scheduleMessage }}</p></div>
      </div>
        ...
        <li v-for="item in schedules" :key="item.task_key" class="schedule-item">
          <div class="schedule-item__info">
            ...
            <p class="schedule-item__meta">
              ...
              <template v-if="item.updated_by"> · 由 {{ item.updated_by }} 于 {{ formatDateTime(item.updated_at) }}
                修改
              </template>
            </p>
          </div>
          <div class="schedule-item__edit">
            ...
            >
              <span class="material-icons-outlined">{{
                  scheduleSaving === item.task_key ? "hourglass_top" : "save"
                }}</span>
    ...
    <section v-if="can('dictionary:sync')" class="sync-note">
      ...
      <div><strong>同步规则</strong>
        <p>科拓区域编码是幂等匹配依据；接口返回为空或失败时不会覆盖本地区域，科拓端已删除的区域也不会被自动删除。</p></div>
    </section>
    <section v-if="can('vehicle-record:sync')" class="sync-note sync-note--records">
      <span class="material-icons-outlined">history</span>
      <div><strong>增量规则</strong>
        <p>仅从上一次成功快照继续读取。图片下载失败会保留源地址并在后续同步中自动重试，不影响流水检查点推进。</p></div>
    </section>
    <section v-if="can('owner:sync')" class="sync-note sync-note--owners">
      <span class="material-icons-outlined">manage_search</span>
      <div><strong>补建规则</strong>
        <p>仅处理近 30
          天有进出记录的车牌，车牌去除间隔符后判重。车主信息以科拓卡片的卡号、姓名和手机号为准，按手机号判重、卡号次之，同一手机号的多个车牌只建一条车主信息并登记多条车牌关联。已有有效档案的车牌直接跳过；科拓没有卡片信息（例如临时车）、车牌档案已停用的记录不会处理。</p>
      </div>
    </section>
    <section v-if="can('account:sync')" class="sync-note sync-note--accounts">
      <span class="material-icons-outlined">key</span>
      <div><strong>账号规则</strong>
        <p>仅处理近 30
          天有进出记录的车牌，车牌去除间隔符后匹配有效车牌档案并关联在营车主。业主手机号即登录名，初始密码统一发放，请提醒业主及时修改。账号部门取自车主信息的部门字段，平台没有同名部门时按名称自动新建（编码以
          AUTO-
          开头），避免账号无处挂靠。缺少车牌档案或车主已停用的记录不会处理，先执行「车主信息补建」补全资料后可再次执行。</p>
      </div>
    </section>
  </section>
</template>
...
async function loadSchedules() {
  ...
  try {
    ...
    data.forEach((item) => {
      scheduleDrafts[item.task_key] = item.cron;
    });
  } catch (error) {
...
function progressFor(...taskKeys: string[]) {
  return progress.value.find(item => taskKeys.includes(item.task_key));
}

function progressPercent(...taskKeys: string[]) {
  const item = progressFor(...taskKeys);
  return item?.total_count ? Math.min(100, Math.round(item.processed_count / item.total_count * 100)) : 8;
}

function progressText(...taskKeys: string[]) {
  const item = progressFor(...taskKeys);
  if (!item) return "正在执行";
  return item.total_count ? `${item.processed_count} / ${item.total_count}（${progressPercent(...taskKeys)}%）` : `已处理 ${item.processed_count} 条`;
}

async function loadProgress() {
  try {
    progress.value = await http.get<SyncProgress[]>("/synchronizations/progress");
  } catch { /* 状态轮询失败不影响手动操作 */
  }
}
...
async function runSynchronization() {
  ...
  try {
    ...
    }).statusMessage || "无法连接同步服务，请检查科拓配置后重试。";
...
async function runAccessRecordSynchronization() {
  ...
  try {
    ...
    accessRecordErrorMessage.value = (error as {
      statusMessage?: string
    }).statusMessage || "无法连接同步服务，请检查科拓配置后重试。";
...
async function runAccountGeneration() {
  ...
  try {
    ...
    accountErrorMessage.value = (error as {
      statusMessage?: string
    }).statusMessage || "无法连接账号生成服务，请稍后重试。";
...
async function runOwnerArchiveGeneration() {
  ...
  try {
    ...
    ownerArchiveErrorMessage.value = (error as {
      statusMessage?: string
    }).statusMessage || "无法连接车主信息补建服务，请检查科拓配置后重试。";
...
async function previewAccessRecordSynchronization() {
  ...
  try {
    ...
    accessRecordErrorMessage.value = (error as {
      statusMessage?: string
    }).statusMessage || "无法获取同步情况，请检查科拓配置后重试。";
    accessRecordSyncState.value = "error";
  } finally {
...
.sync-card__icon .material-icons-outlined {
  font-size: 21px;
}

.sync-card__title-row {
  align-items: center;
  display: flex;
  flex-wrap: wrap;
  gap: 9px;
...
.sync-route {
  ...
  display: grid;
  gap: 16px;
  grid-template-columns: minmax(150px, 1fr) minmax(100px, .8fr) minmax(150px, 1fr);
  margin-top: 24px;
  padding: 18px;
}
...
.sync-route--running .conduit__line {
  background: var(--primary-soft);
}

.sync-route--running .conduit__packet {
  animation: data-flow 1.25s ease-in-out infinite;
  opacity: 1;
}
...
.feedback--error .material-icons-outlined {
  color: var(--red);
}
...
.feedback--preview .material-icons-outlined {
  color: var(--primary);
}
...
.sync-button:focus-visible, .cancel-button:focus-visible {
  outline: 2px solid var(--primary);
  outline-offset: 2px;
}

.sync-button .material-icons-outlined {
  font-size: 17px;
}
...
.result-link .material-icons-outlined {
  font-size: 14px;
}
...
.history-panel__link .material-icons-outlined {
  font-size: 15px;
}
...
.result-panel--records .metric-grid, .result-panel--accounts .metric-grid, .result-panel--owners .metric-grid {
...
.sync-note > .material-icons-outlined {
  color: var(--primary);
  font-size: 18px;
}
...
.sync-button--compact:disabled {
  cursor: not-allowed;
}

.schedule-item__edit .cancel-button {
  padding: 0 10px;
}
...
@media (prefers-reduced-motion: reduce) {
  .sync-route--running .conduit__packet, .spinning {
    animation: none;
  }
...
@media (max-width: 600px) {
  ...
  .sync-card__main, .sync-card__action {
    padding: 18px;
  }
  ...
  .result-panel--records .metric:nth-child(3), .result-panel--accounts .metric:nth-child(3), .result-panel--owners .metric:nth-child(3) {
    border-right: 0;
  }
  ...
  .history-list__time, .history-list__summary {
    flex: 1 1 100%;
  }
