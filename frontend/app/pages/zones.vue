  ...
  <section class="page">
    <header class="page__header">
      ...
      <div class="header-actions">
        <button class="button button--ghost" type="button" @click="goSync"><span class="material-icons-outlined">sync_alt</span>数据同步</button>
        <button class="button button--soft" type="button" @click="refresh"><span class="material-icons-outlined">refresh</span>刷新</button>
      </div>
    </header>

    <section class="lot-card" :class="{ 'lot-card--empty': !lot }">
      <template v-if="lot">
        ...
        <div class="lot-card__metrics">
          ...
          <div class="metric"><span>最近同步</span><strong class="metric__time">{{ formatDateTime(lot.updatedAt) }}</strong></div>
        </div>
      </template>
      <template v-else>
        <div class="lot-card__info">
    ...
    <section class="panel">
      ...
      <div v-else class="table-wrap">
        <table class="table">
          <thead>
            <tr>
              <th>编号</th><th>区域编码</th><th>区域名称</th><th>车位数量</th><th>显示排序</th><th>状态</th><th>更新时间</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="zone in zones" :key="zone.id">
              ...
              <td><span class="status-tag" :class="isActive(zone) ? 'status-tag--normal' : 'status-tag--disabled'">{{ isActive(zone) ? "启用" : "停用" }}</span></td>
              <td class="muted">{{ formatDateTime(zone.updatedAt) }}</td>
            </tr>
            <tr v-if="zones.length === 0"><td class="empty" colspan="7">暂无数据</td></tr>
          </tbody>
        </table>
      </div>
...
interface ZonePage { content?: Zone[]; items?: Zone[]; totalElements?: number; total?: number; page?: { totalElements?: number } }
...
.lot-card { align-items: center; background: var(--card); border: 1px solid var(--border-strong); border-radius: 8px; display: flex; flex-wrap: wrap; gap: 16px; justify-content: space-between; margin-bottom: 16px; padding: 16px 18px; }
.lot-card--empty { border-style: dashed; }
.lot-card__info { align-items: center; display: flex; gap: 12px; }
.lot-card__icon { align-items: center; background: var(--primary-soft); border-radius: 9px; color: var(--primary); display: flex; flex: 0 0 40px; height: 40px; justify-content: center; }
.lot-card__icon .material-icons-outlined { font-size: 21px; }
.lot-card__text h2 { color: var(--text); font-size: 16px; font-weight: 650; margin: 0; }
.lot-card__text p { color: var(--text-sub); margin: 3px 0 0; }
.lot-card__metrics { display: flex; flex-wrap: wrap; gap: 12px; }
.metric { background: var(--bg); border: 1px solid var(--border); border-radius: 7px; min-width: 96px; padding: 10px 14px; }
.metric span { color: var(--text-sub); display: block; font-size: 11px; }
.metric strong { color: var(--text); display: block; font-family: Consolas, "SFMono-Regular", monospace; font-size: 18px; font-variant-numeric: tabular-nums; line-height: 1.3; margin-top: 4px; }
.metric .metric__time { font-family: inherit; font-size: 12px; font-weight: 600; line-height: 1.6; }
...
.code-tag::before { background: var(--text-mute); border-radius: 50%; content: ""; height: 5px; width: 5px; }
.status-tag--normal { background: #f0fdf4; color: #059669; }
.status-tag--normal::before { background: #059669; border-radius: 50%; content: ""; height: 5px; width: 5px; }
.status-tag--disabled { background: #fef2f2; color: #dc2626; }
.status-tag--disabled::before { background: #dc2626; border-radius: 50%; content: ""; height: 5px; width: 5px; }
.muted { color: var(--text-sub) !important; }
...
@media (max-width: 600px) { .page { padding: 16px; }.header-actions { justify-content: flex-end; }.lot-card__metrics { width: 100%; } }
</style>
