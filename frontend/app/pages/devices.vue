  ...
  <section class="page">
    ...
    <section class="card">
      <div class="card__body card__body--filters">
        <div class="toolbar">
          <input v-model.trim="filters.keyword" class="input" placeholder="编号 / 名称 / 品牌 / 位置" type="search"
                 @keyup.enter="search">
          <select v-model="filters.type" aria-label="设备类型" class="select">
            <option value="">全部类型</option>
            <option v-for="type in types" :key="type" :value="type">{{ type }}</option>
          </select>
          <select v-model="filters.status" aria-label="设备状态" class="select">
      ...
      <div v-else class="table-wrap">
        <table class="table">
          ...
          <tr v-for="device in pagedDevices" :key="device.id">
            ...
            <td class="text-sub nowrap">{{ device.lastOnline || "-" }}</td>
            <td><span :class="statusClass(device.status)" class="tag">{{ statusLabel(device.status) }}</span></td>
            <td :title="device.remark || ''" class="remark-cell text-sub">{{ device.remark || "-" }}</td>
            <td class="actions-cell">
              <button v-if="can('device:manage')" class="row-act" title="编辑" type="button" @click="openEdit(device)">
                <span class="material-icons-outlined">edit</span></button>
              <button v-if="can('device:manage')" class="row-act row-act--danger" title="删除" type="button"
                      @click="removeDevice(device)"><span class="material-icons-outlined">delete</span></button>
            </td>
          </tr>
      ...
      <footer v-if="!loading && !errorMessage" class="pagination"><span class="pagination__info">共 {{
        ...
        <button :disabled="page <= 1" class="page-btn" title="上一页" type="button" @click="changePage(page - 1)"><span
        ...
        <button :disabled="page >= totalPages" class="page-btn" title="下一页" type="button"
                @click="changePage(page + 1)"><span class="material-icons-outlined">chevron_right</span></button>
      </footer>
    </section>
    ...
    <div v-if="editorVisible" class="modal-mask" @click.self="editorVisible = false">
      <form class="modal modal--xl" @submit.prevent="saveDevice">
        <header class="modal__head"><h2 class="modal__title">{{ editingId ? "编辑设备" : "新增设备" }}</h2>
        ...
        <div class="modal__body">
          <div class="form-grid">
            <label class="field"><span class="field__label">设备编码<span class="req">*</span></span><input
                v-model.trim="form.code" class="input" placeholder="如：AC-001" required></label><label
              class="field"><span class="field__label">设备名称<span class="req">*</span></span><input
          ...
          <div class="form-grid form-grid--remark"><label class="field full"><span
              class="field__label">备注</span><input v-model.trim="form.remark" class="input"></label></div>
    ...
    <div v-if="toast" :class="`toast--${toast.type}`" class="toast"><span class="material-icons-outlined">{{
        toast.type === 'error' ? 'error_outline' : 'check_circle_outline'
      }}</span>{{ toast.text }}
    </div>
  </section>
</template>
...
const filteredDevices = computed(() => {
  ...
  return devices.value.filter((device) => (!keyword || [device.code, device.name, device.brand, device.location].some((value) => (value || "").toLowerCase().includes(keyword))) && (!filters.type || device.type === filters.type) && (filters.status === "" || device.status === Number(filters.status)));
});
const totalPages = computed(() => Math.max(1, Math.ceil(deviceTotal.value / pageSize)));
const pageNumbers = computed(() => Array.from({length: totalPages.value}, (_, index) => index + 1).slice(Math.max(0, page.value - 3), page.value + 2));
const pagedDevices = computed(() => devices.value);

function statusLabel(status: number | null) {
  return status === 1 ? "在线" : status === 0 ? "离线" : "未连接";
}
...
async function loadDevices() {
  ...
  errorMessage.value = "";
  try {
    const result = await http.get<DeviceList>("/devices", {
    ...
    errorMessage.value = (error as { statusMessage?: string }).statusMessage || "设备数据加载失败";
  } finally {
    loading.value = false;
  }
}
...
async function saveDevice() {
  ...
  try {
    if (editingId.value) await http.put(`/devices/${editingId.value}`, payload, {payloadMode: "json"}); else await http.post("/devices", payload, {payloadMode: "json"});
    editorVisible.value = false;
    await loadDevices();
    showToast(isNew ? "新增成功" : "修改成功", "success");
  } catch (error) {
...
async function removeDevice(device: Device) {
  ...
  try {
    await http.delete(`/devices/${device.id}`);
    await loadDevices();
    showToast("删除成功", "success");
  } catch (error) {
    showToast((error as { statusMessage?: string }).statusMessage || "删除失败", "error");
  }
}
...
.row-act .material-icons-outlined, .icon-button .material-icons-outlined {
  font-size: 16px
}
