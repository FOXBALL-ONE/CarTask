<template>
  <section class="page">
    <header class="page__header">
      ...
      <div class="page__actions">
        <template v-if="activeTab === 'all'">
          <button v-if="can('gate-person:manage')" class="btn btn--ghost btn--sm" type="button" @click="openImport"><span class="material-icons-outlined">upload</span>导入</button>
          <button v-if="can('gate-person:export')" class="btn btn--ghost btn--sm" type="button" :disabled="busy === 'export'" @click="exportPersons"><span class="material-icons-outlined">download</span>{{ busy === "export" ? "导出中..." : "导出" }}</button>
        ...
        <button v-else-if="activeTab === 'pending' && can('gate-person:review')" class="btn btn--ghost btn--sm" type="button" :disabled="busy === 'batch'" @click="approveAll"><span class="material-icons-outlined">done_all</span>{{ busy === "batch" ? "处理中..." : "批量通过" }}</button>
      </div>
    </header>
    ...
    <section class="card">
      <div class="card__body filter-body">
        <div class="toolbar">
          ...
          <select v-if="activeTab === 'all'" v-model="filters.dept" class="select" aria-label="单位" @change="search"><option value="">全部单位</option><option v-for="dept in departmentOptions" :key="dept" :value="dept">{{ dept }}</option></select>
          <select v-if="activeTab === 'all'" v-model="filters.approveStatus" class="select" aria-label="审批状态" @change="search"><option value="">全部审批状态</option><option value="通过">通过</option><option value="审核中">审核中</option><option value="拒绝">拒绝</option></select>
          <select v-if="activeTab === 'all'" v-model="filters.syncStatus" class="select" aria-label="同步状态" @change="search"><option value="">全部同步状态</option><option value="已同步">已同步</option><option value="未同步">未同步</option></select>
          <select v-if="activeTab === 'delete'" v-model="deleteStatus" class="select" aria-label="申请状态" @change="search"><option value="">全部状态</option><option value="待处理">待处理</option><option value="已同意">已同意</option><option value="已拒绝">已拒绝</option></select>
          ...
          <tbody>
            <template v-if="activeTab !== 'delete'">
              <tr v-for="person in visiblePersons" :key="person.id">
                <td><img class="record-face zoomable" :src="faceSrc(person.face)" alt="人脸" @error="useFallbackFace"></td>
                <td>{{ person.code }}</td><td>{{ person.dept }}</td><td>{{ person.name }}</td><td>{{ person.phone }}</td><td>{{ person.idCard }}</td><td>{{ formatDateTime(person.createTime) }}</td>
                ...
                <td class="right actions-cell">
                  ...
                  <template v-if="activeTab === 'all'"><button v-if="can('gate-person:manage')" class="row-act" type="button" title="编辑" @click="openEdit(person)"><span class="material-icons-outlined">edit</span></button><button v-if="can('gate-person:manage')" class="row-act row-act--danger" type="button" title="申请删除" @click="requestDelete(person)"><span class="material-icons-outlined">delete</span></button></template>
                  <template v-else><button v-if="can('gate-person:review')" class="btn btn--primary btn--sm" type="button" @click="approvePerson(person)"><span class="material-icons-outlined">check</span>通过</button><button v-if="can('gate-person:review')" class="btn btn--soft btn--sm" type="button" @click="rejectPerson(person)"><span class="material-icons-outlined">close</span>拒绝</button></template>
                </td>
              </tr>
            </template>
            <tr v-for="request in visibleRequests" v-else :key="request.id">
              <td><img class="record-face zoomable" :src="faceSrc(request.face)" alt="人脸" @error="useFallbackFace"></td>
              <td>{{ request.code }}</td><td>{{ request.dept }}</td><td>{{ request.name }}</td><td>{{ request.phone }}</td><td>{{ request.idCard }}</td><td>{{ request.reason }}</td><td>{{ formatDateTime(request.applyTime) }}</td>
              <td><span class="tag" :class="deleteClass(request.status)">{{ request.status }}</span></td>
              <td class="right actions-cell"><button class="row-act" type="button" title="查看" @click="openDeleteDetail(request)"><span class="material-icons-outlined">visibility</span></button><template v-if="request.status === '待处理'"><button v-if="can('gate-person:review')" class="btn btn--primary btn--sm" type="button" @click="approveDelete(request)"><span class="material-icons-outlined">check</span>同意</button><button v-if="can('gate-person:review')" class="btn btn--soft btn--sm" type="button" @click="rejectDelete(request)"><span class="material-icons-outlined">close</span>拒绝</button></template></td>
            </tr>
            <tr v-if="totalRows === 0"><td class="empty-row" :colspan="activeTab === 'pending' ? 9 : 10">{{ activeTab === 'pending' ? '暂无待审核人员' : activeTab === 'delete' ? '暂无删除申请' : '暂无记录' }}</td></tr>
          </tbody>
        </table>
      </div>
    ...
    <div v-if="detailPerson || detailRequest" class="modal-mask" @click.self="closeDetail">
      <div class="modal modal--detail"><header class="modal__head"><h2 class="modal__title">{{ detailRequest ? '删除申请详情' : '人员详情' }}</h2><button class="icon-btn" type="button" title="关闭" @click="closeDetail"><span class="material-icons-outlined">close</span></button></header><div class="modal__body detail-body"><img class="detail-face" :src="faceSrc((detailPerson || detailRequest)?.face)" alt="人脸照片"><table class="table table--compact"><tbody><tr><td>编号</td><td>{{ (detailPerson || detailRequest)?.code }}</td></tr><tr><td>单位</td><td>{{ (detailPerson || detailRequest)?.dept }}</td></tr><tr><td>用户名</td><td>{{ (detailPerson || detailRequest)?.name }}</td></tr><tr><td>手机号</td><td>{{ (detailPerson || detailRequest)?.phone }}</td></tr><tr><td>身份证</td><td>{{ (detailPerson || detailRequest)?.idCard }}</td></tr><tr v-if="detailRequest"><td>删除原因</td><td>{{ detailRequest.reason }}</td></tr><tr><td>{{ detailRequest ? '申请时间' : '入库时间' }}</td><td>{{ formatDateTime(detailRequest?.applyTime || detailPerson?.createTime) }}</td></tr><tr><td>{{ detailRequest ? '状态' : '审批状态' }}</td><td><span v-if="detailRequest" class="tag" :class="deleteClass(detailRequest.status)">{{ detailRequest.status }}</span><span v-else-if="detailPerson" class="tag" :class="approveClass(detailPerson.approveStatus)">{{ detailPerson.approveStatus }}</span></td></tr><tr v-if="detailPerson"><td>同步状态</td><td>{{ detailPerson.syncStatus }}</td></tr></tbody></table></div><footer class="modal__foot"><button class="btn btn--ghost" type="button" @click="closeDetail">关闭</button></footer></div>
    </div>

    <div v-if="editorVisible" class="modal-mask" @click.self="editorVisible = false"><form class="modal" @submit.prevent="savePerson"><header class="modal__head"><h2 class="modal__title">{{ editingId ? '编辑人员' : '新增人员' }}</h2><button class="icon-btn" type="button" title="关闭" @click="editorVisible = false"><span class="material-icons-outlined">close</span></button></header><div class="modal__body"><div class="form-grid"><label class="field"><span class="field__label">编号<em>*</em></span><input v-model.trim="form.code" class="input" required placeholder="人员编号"></label><label class="field"><span class="field__label">单位<em>*</em></span><input v-model.trim="form.dept" class="input" required placeholder="所属单位" list="gate-depts"><datalist id="gate-depts"><option v-for="dept in departmentOptions" :key="dept" :value="dept"></option></datalist></label><label class="field"><span class="field__label">用户名<em>*</em></span><input v-model.trim="form.name" class="input" required placeholder="请输入用户名"></label><label class="field"><span class="field__label">手机号<em>*</em></span><input v-model.trim="form.phone" class="input" required placeholder="请输入手机号"></label><label class="field"><span class="field__label">身份证<em>*</em></span><input v-model.trim="form.idCard" class="input" required placeholder="18 位身份证号"></label><label class="field field--full"><span class="field__label">人脸照片<em v-if="!editingId">*</em></span><input ref="faceInput" class="input file-input" type="file" accept="image/*" :required="!editingId" @change="selectFace"><span class="field__hint">支持 jpg/png/gif/webp/bmp，不超过 2MB</span></label></div><p v-if="formError" class="form-error">{{ formError }}</p></div><footer class="modal__foot"><button class="btn btn--ghost" type="button" @click="editorVisible = false">取消</button><button class="btn btn--primary" type="submit" :disabled="saving">{{ saving ? '保存中...' : '保存' }}</button></footer></form></div>

    <div v-if="importVisible" class="modal-mask" @click.self="importVisible = false"><div class="modal"><header class="modal__head"><h2 class="modal__title">导入人员信息</h2><button class="icon-btn" type="button" title="关闭" @click="importVisible = false"><span class="material-icons-outlined">close</span></button></header><div class="modal__body"><input ref="importInput" class="input file-input" type="file" accept=".xlsx,.xls" @change="selectImport"><p class="import-hint">使用 Excel 模板导入，列为：人员编号、部门、姓名、手机号、身份证号。导入的人员一律为「审核中 / 未同步」，需在「待审核人员」中审核。样表不含人脸照片，导入后请逐条补充照片。</p><p class="import-hint"><button class="btn btn--ghost btn--sm" type="button" :disabled="busy === 'template'" @click="downloadTemplate"><span class="material-icons-outlined">description</span>{{ busy === "template" ? "下载中..." : "下载导入模板" }}</button></p><p v-if="formError" class="form-error">{{ formError }}</p></div><footer class="modal__foot"><button class="btn btn--ghost" type="button" @click="importVisible = false">取消</button><button class="btn btn--primary" type="button" :disabled="busy === 'import'" @click="importExcel">{{ busy === "import" ? "导入中..." : "确认导入" }}</button></footer></div></div>
  </section>
</template>
...
function loadVisibleFaces() {
  ...
  persons.value.forEach((person) => { if (person.face) needed.add(person.face); void loadFace(person.face); });
  requests.value.forEach((request) => { if (request.face) needed.add(request.face); void loadFace(request.face); });
...
async function loadDepartments() {
  ...
  try {
    ...
    // 拿不到部门字典时退回本页出现过的单位名，筛选框不至于空着。
  }
}
/** 部门字典不可用或没有 department:read 时，用本页实际出现过的单位名兜底。 */
function mergeObservedDepartments() {
  const observed = persons.value.map((person) => person.dept).filter(Boolean);
  if (!observed.length) return;
  departmentOptions.value = [...new Set([...departmentOptions.value, ...observed])].sort();
}
...
async function loadData() {
  ...
  try {
    ...
    mergeObservedDepartments();
    loadVisibleFaces();
  } catch (error) {
    if (requestId === loadSequence) errorMessage.value = (error as { statusMessage?: string }).statusMessage || "门禁人员数据加载失败";
  }
  finally { if (requestId === loadSequence) loading.value = false; }
}
async function search() { page.value = 1; successMessage.value = ""; await loadData(); }
...
function nextCode() {
  if (filters.keyword || filters.dept || filters.approveStatus || filters.syncStatus) return "";
...
function openCreate() { editingId.value = null; Object.assign(form, { code: nextCode(), dept: "", name: "", phone: "", idCard: "" }); selectedFace.value = null; formError.value = ""; editorVisible.value = true; }
...
function selectFace(event: Event) {
  ...
  if (file && file.size > 2 * 1024 * 1024) { formError.value = "人脸照片不能超过 2MB"; selectedFace.value = null; (event.target as HTMLInputElement).value = ""; return; }
  formError.value = "";
  selectedFace.value = file;
}
function selectImport(event: Event) { selectedImport.value = (event.target as HTMLInputElement).files?.[0] || null; formError.value = ""; }

async function savePerson() {
  if (!form.code || !form.dept || !form.name || !form.phone || !form.idCard || (!editingId.value && !selectedFace.value)) { formError.value = "编号、单位、用户名、手机号、身份证和人脸照片不能为空"; return; }
...
async function requestDelete(person: Person) { const reason = window.prompt(`请输入删除“${person.name}”的原因`); if (!reason?.trim()) return; actionError.value = ""; try { await http.post(`/gate-persons/${person.id}/delete-requests`, { reason: reason.trim() }, { payloadMode: "json" }); successMessage.value = "删除申请已提交，等待审核"; await loadData(); } catch (error) { actionError.value = (error as { statusMessage?: string }).statusMessage || "提交删除申请失败"; } }
async function approvePerson(person: Person) { await updateApproval(person, "approve", undefined); }
async function rejectPerson(person: Person) { const reason = window.prompt(`请输入拒绝“${person.name}”的原因（可留空）`); if (reason === null) return; await updateApproval(person, "reject", reason.trim() || undefined); }
async function updateApproval(person: Person, action: "approve" | "reject", reason?: string) {
  busy.value = `review-${person.id}`; successMessage.value = ""; actionError.value = "";
  try {
    await http.put(`/gate-persons/${person.id}/${action}`, { reason });
    successMessage.value = action === "approve" ? `已通过【${person.name}】` : `已拒绝【${person.name}】`;
    await loadData();
  } catch (error) { actionError.value = (error as { statusMessage?: string }).statusMessage || "审批操作失败"; }
  finally { busy.value = ""; }
}
...
async function approveAll() {
  ...
  busy.value = "batch"; successMessage.value = ""; actionError.value = "";
  const ids = await pendingIds().catch(() => [] as number[]);
  if (!ids.length) { successMessage.value = "没有待审核人员"; busy.value = ""; return; }
  if (!window.confirm(`确认批量通过 ${ids.length} 名待审核人员？`)) { busy.value = ""; return; }
  let reviewed = 0;
  try {
    ...
    for (let start = 0; start < ids.length; start += BATCH_REVIEW_SIZE) {
      ...
      const result = await http.put<{ reviewed: number }>("/gate-persons/reviews", { ids: chunk, approved: true }, { payloadMode: "json" });
  ...
  } finally { await loadData(); busy.value = ""; }
}
async function approveDelete(request: DeleteRequest) { await updateDeleteRequest(request, "approve"); }
async function rejectDelete(request: DeleteRequest) { await updateDeleteRequest(request, "reject"); }
async function updateDeleteRequest(request: DeleteRequest, action: "approve" | "reject") {
  busy.value = `delete-${request.id}`; successMessage.value = ""; actionError.value = "";
  try {
    ...
    await loadData();
  } catch (error) { actionError.value = (error as { statusMessage?: string }).statusMessage || "删除申请处理失败"; }
  finally { busy.value = ""; }
}
function openImport() { selectedImport.value = null; formError.value = ""; successMessage.value = ""; importVisible.value = true; }
async function downloadTemplate() {
  ...
  } catch (error) { formError.value = error instanceof Error ? error.message : "模板下载失败"; }
  finally { busy.value = ""; }
}
async function importExcel() {
  if (!selectedImport.value) { formError.value = "请选择 Excel 文件"; return; }
  busy.value = "import"; formError.value = "";
  try {
    const body = new FormData(); body.append("file", selectedImport.value);
    ...
    await loadData();
  } catch (error) { formError.value = error instanceof Error ? error.message : "导入失败"; }
  finally { if (importInput.value) importInput.value.value = ""; busy.value = ""; }
}
/** 导出走后端的全量导出：前端只导当前页会把绝大部分数据静默丢掉。 */
async function exportPersons() {
  busy.value = "export"; actionError.value = ""; successMessage.value = "";
  try {
  ...
  } catch (error) { actionError.value = error instanceof Error ? error.message : "导出失败"; }
  finally { busy.value = ""; }
}
...
onMounted(async () => { await Promise.all([loadDepartments(), loadData()]); });
onBeforeUnmount(() => { Object.values(faceObjectUrls.value).forEach((url) => URL.revokeObjectURL(url)); });
