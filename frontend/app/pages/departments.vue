    ...
    <div v-if="editorVisible" class="modal-mask" @click.self="editorVisible = false">
      <form class="modal" @submit.prevent="saveDepartment">
        ...
        <div class="modal__body">
          <div class="form-grid">
            <label class="field"><span class="field__label">部门名称<em>*</em></span><input v-model.trim="form.name"
                                                                                            class="input"
                                                                                            required></label>
            <label class="field"><span class="field__label">部门编码<em>*</em></span><input v-model.trim="form.code"
                                                                                            class="input"
                                                                                            required></label>
            <label class="field"><span class="field__label">上级部门</span><select v-model.number="form.parent"
                                                                                   class="select">
            ...
            <label class="field"><span class="field__label">显示排序</span><input v-model.number="form.sort"
                                                                                  class="input" min="1"
                                                                                  type="number"></label>
            <label class="field"><span class="field__label">负责人</span><input v-model.trim="form.leader"
                                                                                class="input"></label>
            <label class="field"><span class="field__label">联系电话</span><input v-model.trim="form.phone"
                                                                                  class="input"></label>
            <label class="field full"><span class="field__label">状态</span><select v-model.number="form.status"
                                                                                    class="select">
  ...
  setup(props: DepartmentNodeProps, {emit}) {
    ...
    return () => h("li", {class: "tree-node"}, [
      h("div", {class: "tree-row", style: {paddingLeft: `${10 + props.level * 16}px`}}, [
        h("button", {
          class: ["tree-toggle", {open: props.open.has(props.department.id)}],
          type: "button",
          title: hasChildren.value ? "展开/折叠" : undefined,
          onClick: () => hasChildren.value && emit("toggle", props.department.id)
        }, [
        ...
        canManageDepartment.value ? h("button", {
          class: "row-act",
          type: "button",
          title: "编辑",
          onClick: (event: Event) => {
        ...
        }, [h("span", {class: "material-icons-outlined"}, "edit")]) : null,
        canManageDepartment.value ? h("button", {
          ...
          onClick: (event: Event) => {
            event.stopPropagation();
            emit("remove", props.department.id);
          }
        }, [h("span", {class: "material-icons-outlined"}, "delete")]) : null,
      ]),
      hasChildren.value && props.open.has(props.department.id)
          ? h("ul", {class: "tree tree-children open"}, children.value.map((child) => h(DepartmentNode, {
            department: child,
...
function descendantIds(id: number | null) {
  ...
  while (pending.length) {
    ...
    departments.value.filter((item) => item.parent === current).forEach((item) => {
      result.add(item.id);
...
.tree-toggle:not(.open) .material-icons-outlined {
  transform: none;
}

.tree-toggle.open .material-icons-outlined {
  transform: rotate(90deg);
}

.tree-toggle .material-icons-outlined {
  font-size: 16px;
...
:deep(.tree-toggle .material-icons-outlined) {
  font-size: 16px;
  transition: transform var(--tr);
}

:deep(.tree-toggle.open .material-icons-outlined) {
  transform: rotate(90deg);
}
...
:deep(.row-act .material-icons-outlined), :deep(.icon-button .material-icons-outlined) {
  font-size: 16px;
}
