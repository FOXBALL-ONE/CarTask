...
@Service
/** 基于 JPA 的车牌类型字典服务。 */
class LicensePlateTypeServiceImpl(
    private val repository: LicensePlateTypeRepository,
) : LicensePlateTypeService {
    @Transactional
    override fun create(entity: LicensePlateType): LicensePlateType {
        require(entityId(entity) == null) { "创建记录时不能指定 ID" }
        return repository.save(entity)
    }

    @Transactional
    override fun createBatch(entities: List<LicensePlateType>): List<LicensePlateType> {
    ...
    @Transactional
    override fun get(id: Long): LicensePlateType = repository.findById(id)
        .orElseThrow { IllegalArgumentException("记录不存在: $id") }

    @Transactional
    override fun getBatch(ids: List<Long>): List<LicensePlateType> {
    ...
    @Transactional
    override fun list(page: Int, pageSize: Int): Page<LicensePlateType> {
    ...
    @Transactional
    override fun update(id: Long, entity: LicensePlateType): LicensePlateType {
    ...
    @Transactional
    override fun updateBatch(entities: List<LicensePlateType>): List<LicensePlateType> {
    ...
    private fun entityId(entity: LicensePlateType): Long? {
    ...
    private fun copyEditableProperties(source: LicensePlateType, target: LicensePlateType) {
