...
class ReleaseTypeServiceImpl(
    private val repository: ReleaseTypeRepository,
) : ReleaseTypeService {
    @Transactional
    override fun create(entity: ReleaseType): ReleaseType {
        require(entityId(entity) == null) { "创建记录时不能指定 ID" }
        return repository.save(entity)
    }

    @Transactional
    override fun createBatch(entities: List<ReleaseType>): List<ReleaseType> {
    ...
    @Transactional
    override fun get(id: Long): ReleaseType = repository.findById(id)
        .orElseThrow { IllegalArgumentException("记录不存在: $id") }

    @Transactional
    override fun getBatch(ids: List<Long>): List<ReleaseType> {
    ...
    @Transactional
    override fun list(page: Int, pageSize: Int): Page<ReleaseType> {
    ...
    @Transactional
    override fun update(id: Long, entity: ReleaseType): ReleaseType {
    ...
    @Transactional
    override fun updateBatch(entities: List<ReleaseType>): List<ReleaseType> {
    ...
    private fun entityId(entity: ReleaseType): Long? {
    ...
    private fun copyEditableProperties(source: ReleaseType, target: ReleaseType) {
