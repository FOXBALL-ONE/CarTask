...
@Service
/** 基于 JPA 的区域类型字典服务。 */
class ZoneTypeServiceImpl(
    private val repository: ZoneTypeRepository,
) : ZoneTypeService {
    @Transactional
    override fun create(entity: ZoneType): ZoneType {
        require(entityId(entity) == null) { "创建记录时不能指定 ID" }
        return repository.save(entity)
    }

    @Transactional
    override fun createBatch(entities: List<ZoneType>): List<ZoneType> {
    ...
    @Transactional
    override fun get(id: Long): ZoneType = repository.findById(id)
        .orElseThrow { IllegalArgumentException("记录不存在: $id") }

    @Transactional
    override fun getBatch(ids: List<Long>): List<ZoneType> {
    ...
    @Transactional
    override fun list(page: Int, pageSize: Int): Page<ZoneType> {
    ...
    @Transactional
    override fun update(id: Long, entity: ZoneType): ZoneType {
    ...
    @Transactional
    override fun updateBatch(entities: List<ZoneType>): List<ZoneType> {
    ...
    private fun entityId(entity: ZoneType): Long? {
    ...
    private fun copyEditableProperties(source: ZoneType, target: ZoneType) {
