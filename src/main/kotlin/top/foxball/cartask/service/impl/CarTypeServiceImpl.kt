...
@Service
/** 基于 JPA 的车辆类型字典服务。 */
class CarTypeServiceImpl(
    private val repository: CarTypeRepository,
) : CarTypeService {
    @Transactional
    override fun create(entity: CarType): CarType {
        require(entityId(entity) == null) { "创建记录时不能指定 ID" }
        return repository.save(entity)
    }

    @Transactional
    override fun createBatch(entities: List<CarType>): List<CarType> {
    ...
    @Transactional
    override fun get(id: Long): CarType = repository.findById(id)
        .orElseThrow { IllegalArgumentException("记录不存在: $id") }

    @Transactional
    override fun getBatch(ids: List<Long>): List<CarType> {
    ...
    @Transactional
    override fun list(page: Int, pageSize: Int): Page<CarType> {
    ...
    @Transactional
    override fun update(id: Long, entity: CarType): CarType {
    ...
    @Transactional
    override fun updateBatch(entities: List<CarType>): List<CarType> {
    ...
    private fun entityId(entity: CarType): Long? {
    ...
    private fun copyEditableProperties(source: CarType, target: CarType) {
