...
@Service
/** 基于 JPA 的门禁授权类型服务。 */
class AccessControlTypeServiceImpl(
    private val repository: AccessControlTypeRepository,
) : AccessControlTypeService {
    @Transactional
    override fun create(entity: AccessControlType): AccessControlType {
        require(entityId(entity) == null) { "创建记录时不能指定 ID" }
        return repository.save(entity)
    }

    @Transactional
    override fun createBatch(entities: List<AccessControlType>): List<AccessControlType> {
    ...
    @Transactional
    override fun get(id: Long): AccessControlType = repository.findById(id)
        .orElseThrow { IllegalArgumentException("记录不存在: $id") }

    @Transactional
    override fun getBatch(ids: List<Long>): List<AccessControlType> {
    ...
    @Transactional
    override fun list(page: Int, pageSize: Int): Page<AccessControlType> {
    ...
    @Transactional
    override fun update(id: Long, entity: AccessControlType): AccessControlType {
    ...
    @Transactional
    override fun updateBatch(entities: List<AccessControlType>): List<AccessControlType> {
    ...
    private fun entityId(entity: AccessControlType): Long? {
    ...
    private fun copyEditableProperties(source: AccessControlType, target: AccessControlType) {
