...
@Service
/** 基于 JPA 的限制类型字典服务。 */
class RestrictionTypeServiceImpl(
    private val repository: RestrictionTypeRepository,
) : RestrictionTypeService {
    @Transactional
    override fun create(entity: RestrictionType): RestrictionType {
        require(entityId(entity) == null) { "创建记录时不能指定 ID" }
        return repository.save(entity)
    }

    @Transactional
    override fun createBatch(entities: List<RestrictionType>): List<RestrictionType> {
    ...
    @Transactional
    override fun get(id: Long): RestrictionType = repository.findById(id)
        .orElseThrow { IllegalArgumentException("记录不存在: $id") }

    @Transactional
    override fun getBatch(ids: List<Long>): List<RestrictionType> {
    ...
    @Transactional
    override fun list(page: Int, pageSize: Int): Page<RestrictionType> {
    ...
    @Transactional
    override fun update(id: Long, entity: RestrictionType): RestrictionType {
    ...
    @Transactional
    override fun updateBatch(entities: List<RestrictionType>): List<RestrictionType> {
    ...
    private fun entityId(entity: RestrictionType): Long? {
    ...
    private fun copyEditableProperties(source: RestrictionType, target: RestrictionType) {
